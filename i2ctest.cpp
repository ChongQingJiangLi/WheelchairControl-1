...
// Included for hex to voltage conversion.
#include <sstream>
...
// THIS SECTION NEED VERIFICATION NAMELY TOP VOLTAGE OUTPUT FOR DAC.

// For wheelchair maximum voltage should be 4.0 V, assuming 5V output max:
// The hex value of fff will correspond to 5V with each bit representing 
// 1.22 mV 
// 0V (Full reverse):   0x000
// 1.5V (Stop): 	0x4cc
// 4.0V (Full Forward): 0xccc
// 5.0V (Max value):    0xfff


// Sawtooth wave is generated and sent to the DAC. This should produce from full reverse 
// to full forward in the chair
...
// Phase two for test, low power is generated.
// Chair should start from 0 and move to full forward. (actually max voltage at this point)

 for(int i = 0x000004cc; i < 0x0000ffff; i++)
   {
      result = wiringPiI2CWriteReg16(fd, 0x40, (i & 0xfff) );
...
// Test Wheelchair sensitivity to granularity of DAC.
// Step through values slowly until motion. Print value to console for observation.

   for(int i = 0x000004cc; i < 0x0000ffff; i++)
   {
      result = wiringPiI2CWriteReg16(fd, 0x40, (i & 0xfff) );
	cout <<	"Current chair input is:" << (i & 0xfff) << endl;
...
// Stop Chair
result = wiringPiI2CWriteReg16(fd, 0x40, (0x000004cc));
